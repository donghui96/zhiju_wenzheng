...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagchuang)
    {
        info->event[info->num] = EVENT_chuang;
        info->num++;
        dataPoints->valuechuang = gizStandardDecompressionValue(chuang_BYTEOFFSET,chuang_BITOFFSET,chuang_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagled_1)
    {
        info->event[info->num] = EVENT_led_1;
        info->num++;
        dataPoints->valueled_1 = gizStandardDecompressionValue(led_1_BYTEOFFSET,led_1_BITOFFSET,led_1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagled_2)
    {
        info->event[info->num] = EVENT_led_2;
        info->num++;
        dataPoints->valueled_2 = gizStandardDecompressionValue(led_2_BYTEOFFSET,led_2_BITOFFSET,led_2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagreshuiqi)
    {
        info->event[info->num] = EVENT_reshuiqi;
        info->num++;
        dataPoints->valuereshuiqi = gizStandardDecompressionValue(reshuiqi_BYTEOFFSET,reshuiqi_BITOFFSET,reshuiqi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagjinghuaqi)
    {
        info->event[info->num] = EVENT_jinghuaqi;
        info->num++;
        dataPoints->valuejinghuaqi = gizStandardDecompressionValue(jinghuaqi_BYTEOFFSET,jinghuaqi_BITOFFSET,jinghuaqi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagtiaoguang)
    {
        info->event[info->num] = EVENT_tiaoguang;
        info->num++;
        dataPoints->valuetiaoguang = gizX2Y(tiaoguang_RATIO,  tiaoguang_ADDITION, issuedData->attrVals.valuetiaoguang); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuechuang != cur->valuechuang)
    {
        GIZWITS_LOG("valuechuang Changed\n");
        ret = 1;
    }
    if(last->valueled_1 != cur->valueled_1)
    {
        GIZWITS_LOG("valueled_1 Changed\n");
        ret = 1;
    }
    if(last->valueled_2 != cur->valueled_2)
    {
        GIZWITS_LOG("valueled_2 Changed\n");
        ret = 1;
    }
    if(last->valuereshuiqi != cur->valuereshuiqi)
    {
        GIZWITS_LOG("valuereshuiqi Changed\n");
        ret = 1;
    }
    if(last->valuejinghuaqi != cur->valuejinghuaqi)
    {
        GIZWITS_LOG("valuejinghuaqi Changed\n");
        ret = 1;
    }
    if(last->valuetiaoguang != cur->valuetiaoguang)
    {
        GIZWITS_LOG("valuetiaoguang Changed\n");
        ret = 1;
    }
    ...
    if(last->valueshidu != cur->valueshidu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueshidu Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(chuang_BYTEOFFSET,chuang_BITOFFSET,chuang_LEN,(uint8_t *)devStatusPtr,dataPoints->valuechuang);
    gizStandardCompressValue(led_1_BYTEOFFSET,led_1_BITOFFSET,led_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueled_1);
    gizStandardCompressValue(led_2_BYTEOFFSET,led_2_BITOFFSET,led_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueled_2);
    gizStandardCompressValue(reshuiqi_BYTEOFFSET,reshuiqi_BITOFFSET,reshuiqi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuereshuiqi);
    gizStandardCompressValue(jinghuaqi_BYTEOFFSET,jinghuaqi_BITOFFSET,jinghuaqi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuejinghuaqi);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valuetiaoguang = gizY2X(tiaoguang_RATIO,  tiaoguang_ADDITION, dataPoints->valuetiaoguang); 
    devStatusPtr->valuewendu = gizY2X(wendu_RATIO,  wendu_ADDITION, dataPoints->valuewendu); 
    devStatusPtr->valueshidu = gizY2X(shidu_RATIO,  shidu_ADDITION, dataPoints->valueshidu); 
